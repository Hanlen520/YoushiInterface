...
class PadLiveReserve(PubTest):
    ...
    def test_Reserve_case(self):
        code = requests.request("POST", url=PubTest.PadLiveReserve, data=json.dumps(self.play))
    ...
    def test_reserve_seek_case(self):
        dd = deepcopy(self.play)
        code = requests.request("POST", url=PubTest.PadLiveReserve, data=self.play)
        ss =json.loads(code.text)
